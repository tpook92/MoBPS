...
#' population <- breeding.diploid_old(population, breeding.size=100, selection.size=c(25,25))
#' @return Population-list
#' @export


breeding.diploid_old <- function(population,
            mutation.rate = 10^-8,
  ...
  if(length(pen.size)==0){
    ...
    if(verbose & pen.size.overwrite){
      warning("New standard for litter size / repeat.mating.copy set. This will be the new default for all downstream generation of offspring via copy/combine.")
  ...
  #######################################################################
{
  ...
  # Check if underlying true genomic values need to be calculated ((usually only on first use of breeding.diploid_old() or when traits are added))
        ...
        if(mas.bve){
          ...
          if(!bve.beta.hat.approx){
            if(population$info$phenotypic.transform[bven]==FALSE || min(y_obs[,bven])>0){
              ...
              pop1 <- breeding.diploid_old(population, phenotyping.database = loop_elements[,c(4,5,2)], verbose=TRUE)
