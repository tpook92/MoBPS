...
get.plink <- function(population, path=NULL, database=NULL, gen=NULL, cohorts=NULL, chromosome="all",
                      non.genotyped.as.missing=FALSE, fam.id = FALSE, type = 0){

  database = get.database(population, gen = gen, database = database, cohorts = cohorts)
  nindi = get.nindi(population, database = database)
  if(type ==0){
    geno <- get.geno(population, database=database, chromosome=chromosome, export.alleles=FALSE, use.id = TRUE)

  } else if(type ==1){
    geno <- get.haplo(population, database=database, chromosome=chromosome, export.alleles=FALSE, use.id = TRUE)[,1:nindi*2-1]

    colnames(geno) = substr(colnames(geno), start = 1, stop = nchar(colnames(geno))-5)
  } else if(type ==2){
    geno <- get.haplo(population, database=database, chromosome=chromosome, export.alleles=FALSE, use.id = TRUE)[,1:nindi*2]
    colnames(geno) = substr(colnames(geno), start = 1, stop = nchar(colnames(geno))-5)
  ...
  options("scipen"=999)
  ped_tmp = get.pedigree(population, database = database, id = TRUE, include.error = bve.pedigree.error)
