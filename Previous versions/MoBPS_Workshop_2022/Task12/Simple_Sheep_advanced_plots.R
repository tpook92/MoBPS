...
for(scenario in 1:2){
  for(run in 1:n_run){
    ...
    load(paste0(paste0("C:/Users/pook/Desktop/MoBPS_workshop_gft/Task12/Sheep_simulation_scenario", scenario, "run", run,".RData")))
...
par(mfrow=c(1,2))
for(trait in 1:2){
  plot(0:20, ACC_avg[[1]][trait,to_plot], type="l", main=traitname[trait], xlab="cycle", ylab="genomic value", lwd=2, ylim=c(0,1))
  for(sc in 2:4){
    lines(0:20, ACC_avg[[sc]][trait,to_plot], col=sc, type="l", xlab="cycle", ylab="genomic value", lwd=2)
  }
}
