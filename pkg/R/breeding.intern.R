...
breeding.intern <- function(info.parent, parent,  population , mutation.rate, remutation.rate, recombination.rate,
                            ...
                            duplication.recombination, delete.same.ursprung=FALSE,
...
#    activ.dup3 <- (dup[[activ]][,1] < porc[index+1]) * (dup[[activ]][,1] >= porc[index]) * (dup[[activ]][,8] == 3)
    ...
    activ.ursprung <- unique(c(0,start:(start+sum(activ.porc))))[-1]
    if(length(activ.ursprung)>0){
      while(max(activ.ursprung,-1)>length(parent[[activ+4]])){ # add -1 avoid max(numeric(0))
        activ.ursprung <- activ.ursprung[-length(activ.ursprung)]
    ...
    if(porc[index+1]>0 && save1){
      new.ursprung <- c(new.ursprung, parent[[activ+4]][activ.ursprung])
    }
 ...
 #   first.following[length(first.following)] <- 1
  #  activ.porc[which(first.following == 1)[1]] <- 1
   # activ.porc <- activ.porc * 1:length(activ.porc)
    #new.ursprung <- rbind(new.ursprung, parent[[activ+4]][activ.porc,])
    activ <- 3- activ
  }
...
#    print(new.dups)
  ...
  new.ursprung_old <- new.ursprung
  new.poc_old <- new.poc
  if(delete.same.ursprung==TRUE && length(new.ursprung)>1){
    for(index in length(new.ursprung):2){
      check <- prod(new.ursprung[index] == new.ursprung[index-1])
      if(check==TRUE){
        new.ursprung <- new.ursprung[-index]
  ...
  if(length(new.poc)!=(length(new.ursprung)+1)){
    print("Rekombination - Ursprung Inkonsistenz!!")
      print(new.poc)
      print(new.ursprung)
      print(parent)
      print(porc)
  ...
  if(gene.editing==TRUE){
    hap_sequence <- compute.snps_single(population, new.poc, new.mut, new.ursprung, decodeOriginsU=decodeOriginsU)
  ...
  return(list(new.poc, new.mut, new.ursprung, info.parent, new.dup, porc, share_a))
}
