...
get.pedmap <- function(population, path=NULL, database=NULL, gen=NULL, cohorts=NULL, chromosomen="all"){
  ...
  write.table(file=pedname, pedfile, col.names = FALSE, row.names = FALSE, quote=FALSE)
  mapname <- paste0(path,".map")
  write.table(file=mapname, mapfile, col.names = FALSE, row.names = FALSE, quote=FALSE)
}
