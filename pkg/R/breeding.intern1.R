...
#' @examples
#' data(ex_pop)
#' child_gamete <- breeding.intern1(info.parent = c(1,1,1), parent = ex_pop$breeding[[1]][[1]][[1]],
#'                                 population = ex_pop)
...
breeding.intern1 <- function(info.parent, parent,  population , mutation.rate = 10^-5, remutation.rate = 10^-5, recombination.rate=1,
  ...
  activ_rec <- list()
  for(activ in 1:2){
    activ.porc  <- rep(FALSE, length(parent[[activ]]))
    act_rec <- 1
    if(activ==1){
      temp1 <- FALSE
    } else{
    ...
    for(index in 1:length(activ.porc)){
      while(porc[act_rec]<=parent[[activ]][index]){
        temp1 <- !temp1
        act_rec <- act_rec +1
      }
      activ.porc[index] <- temp1
    }
    activ_rec[[activ]] <- activ.porc
  }

  ori_temp <- as.integer(numeric(length(porc)))
  activ <- 1
  for(index in 1:length(porc)){
    temp1 <- sum(parent[[activ]]<porc[index])

    if(temp1>0){
      ori_temp[index] <- parent[[activ+4]][temp1]
    }

    activ <- 3 - activ
  }

  new.poc1 <- c( parent[[1]][activ_rec[[1]]], parent[[2]][activ_rec[[2]]], porc[-c(1,length(porc))])

  new.origin1 <- c( parent[[5]][activ_rec[[1]]], parent[[6]][activ_rec[[2]]], ori_temp[-c(1,length(ori_temp))])

  sorting <- sort(new.poc1, index.return=TRUE)$ix

  new.poc2 <- new.poc1[sorting]
  new.origin2 <- new.origin1[sorting]

  temp1 <- !duplicated(new.poc2)

  new.poc <- new.poc2[temp1]
  new.origin3 <- new.origin2[temp1]
  new.origin <- new.origin3[-length(new.origin3)]
