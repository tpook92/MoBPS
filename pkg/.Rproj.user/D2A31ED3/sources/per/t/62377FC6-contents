...
pedmap.to.phasedbeaglevcf <- function(ped_path=NULL, map_path=NULL, beagle_jar="beagle.03Jul18.40b.jar", plink_dir="./Plink/plink", db_dir="Plink/DB/"){
  map <- read.table(map_path)
  ...
  if(changes>0 || length(remove_dup)>0){
    ...
    write.table(file=new_map_name, map, quote=FALSE, row.names = FALSE, col.names=FALSE)
  } else{
  ...
  if(length(remove_dup)>0){
    ...
    write.table(file=new_ped_name, ped, quote=FALSE, row.names = FALSE, col.names=FALSE)
  ...
  plink_code <- paste0(plink_dir," --noweb --dog --map ",new_map_name," --ped ",new_ped_name," --maf 0.0001 --recode vcf --out Plink/DB/data")
  system(plink_code)

  # Use Beagle 5.0 to generated phased + imputed dataset
  beagle_code <- paste0("java -jar ",beagle_jar," gt=",db_dir,"data.vcf out=",db_dir,"data_phased")
  system(beagle_code)
}
