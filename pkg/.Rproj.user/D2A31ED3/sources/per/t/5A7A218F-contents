...
creating.diploid <- function(dataset=NULL, vcf=NULL, chr.nr=NULL, bp=NULL, snp.name=NULL, hom0=NULL, hom1=NULL,
  ...
  if(skip.rest==FALSE){
    ...
    # To little gain by miraculix to justify direct sampling in C.
    # If memory in generation becomes major issue reconsider implementation!
    if(length(chr.nr)>0){
      chr.opt <- unique(chr.nr)
  ...
  if(is.matrix(dataset) && is.numeric(dataset[1,1]) && storage.mode(dataset)!= "integer"){
