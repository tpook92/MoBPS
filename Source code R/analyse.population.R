...
#' Analyse allele frequency of a single marker
#'
#' Analyse allele frequency of a single marker
...
analyse.population <- function(population, chromosome, snp, include=c(1,2)){
...
#    active.animals <- NULL
  ...
  maxp <- max(state.prob)
  graphics::plot(1:generations ,state.prob[1,],xlim=c(1,generations),ylim=c(0,maxp),type="l",xlab="Generationen", ylab="Anteil")
  graphics::lines(1:generations ,state.prob[2,],lty=2)
  graphics::lines(1:generations ,state.prob[3,],lty=3)
  graphics::legend("topleft",legend = c("hom0","hetero","hom1"),lty=c(1,2,3))
return(state)
}
